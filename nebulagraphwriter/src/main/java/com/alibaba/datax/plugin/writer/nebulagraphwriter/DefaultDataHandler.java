...
public class DefaultDataHandler implements DataHandler {
    ...
    public DefaultDataHandler(Configuration conf, TaskPluginCollector taskPluginCollector) {
        ...
        this.hostPorts = conf.getList(Key.HOST_PORTS, String.class);
    ...
    public int handle(RecordReceiver rec, TaskPluginCollector collector) {
        ...
        try {
            ...
            List<HostAddress> addressList = hostPorts.stream().map(s -> {
                String[] hostArr = s.split(":");
                return new HostAddress(hostArr[0], Integer.parseInt(hostArr[1]));
            ...
            NebulaDriver nebulaDriver =  new NebulaDriver(poolProperties);
            // get Connection
            Connection conn = DriverManager.getConnection(jdbcUrl, username, password);
            //DriverManager.getConnection(jdbcUrl, username, password);
            String graphSpace = jdbcUrl.split("//")[1];
            Session session = getSession(hostPorts,username,password,graphSpace,nebulaDriver.getNebulaPoolConfig());
    ...
    private Session getSession(List<String> addresList, String userName, String password, String graphSpace, NebulaPoolConfig nebulaPoolConfig) throws UnknownHostException, IOErrorException, AuthFailedException, ClientServerIncompatibleException, NotValidConnectionException, UnsupportedEncodingException {
        // 利用nebula-java连接nebulaGraph
        List<HostAddress> addresses = addresList.stream().map(s -> {
            String[] hostArr = s.split(":");
            return new HostAddress(hostArr[0], Integer.parseInt(hostArr[1]));
        }).collect(Collectors.toList());
        ...
        session.execute("use " + graphSpace); // throws声明了Exception是否还需要try-catch处理
        return session;
    }
